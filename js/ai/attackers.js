...
import { ATT_START_SECTORS } from "../constants.js";


// --- helper: zajętość kratki ---
function occupied(game, x, y, selfId = null) {
  return [...game.attackers, ...game.defenders]
    .some(u => u.hp > 0 && u.x === x && u.y === y && u.id !== selfId);
}
...
// Docelowe komórki flagi (w sektorze 63)
const FLAG_CELLS = [
  { x: 16, y: 9 },
  { x: 16, y: 10 },
  { x: 17, y: 9 },
...
export function aiPlanMoves(game) {
  const plans = {};
...
// --- auto ensure cell→sector map
  if (!(game.cellToSector instanceof Map)) game.cellToSector = new Map();
  if (game.cellToSector.size === 0) {
    // importuj na górze pliku: import { buildCellMap } from "../sectors.js";
    buildCellMap(game);
  }
  ...
  function _isCellFree(x, y, exclude=null){
    return ![...game.attackers, ...game.defenders]
      .some(u => u.hp>0 && u.x===x && u.y===y && String(u.id)!==String(exclude));
  }
  ...
  function pickBestCellInSectorTowardsFlag(unit, targetSec, reserved){
    ...
    for (const c of sorted){
      ...
      if (_isCellFree(c.x, c.y, unit.id)) {
        reserved.add(key);
        return { x: c.x, y: c.y };
      }
    }
  ...
  for (const id of ids) {
    ...
    if (d.x != null && d.y != null) {
      const key = `${d.x}-${d.y}`;
      if (!_isCellFree(d.x, d.y, u.id)) continue;
    ...
    if (d.sec != null) {
      ...
      const cell = pickBestCellInSectorTowardsFlag(u, targetSec, reserved);
      if (!cell) continue;
...
function _isCellFree(game, x, y, exclude=null){
  return ![...game.attackers, ...game.defenders]
    .some(u => u.hp>0 && u.x===x && u.y===y && u.id!==exclude);
}
...
export function placeAttackersClustered(game, count = 5){
  const startSectors = ATT_START_SECTORS.map(String);

  function sectorScore(sid){
    const cells = enumerateSectorCells(sid).filter(([x,y]) => _isCellFree(game, x, y));
  ...
  const free = enumerateSectorCells(chosenSector)
    .filter(([x,y]) => _isCellFree(game, x, y))
    .sort((a,b)=>{
      const da = Math.min(
...
function _sectorsOrderFromBestStart(game){
  const allowed = ATT_START_SECTORS.map(String);
  const rnd = _rng();
...
export function placeAttackersAuto(game, count = 5){
  ...
  for (const sec of sectorsOrder){
    ...
    for (const [x,y] of byGate(sec)){
      if (out.length >= want) break;
      if (_isCellFree(game, x, y)) out.push({ x, y });
    }
  }
