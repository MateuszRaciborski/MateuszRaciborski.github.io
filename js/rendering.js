...
import { GRID_W, GRID_H, CELL } from './constants.js';
...
window.drawAnimatedFlag = drawAnimatedFlag;
...
window.drawBulletProjectile = drawBulletProjectile;
...
export function getAimDodgeAt(x, y, game){
  ...
  switch (sec.type){
    case 'flag':     return { aim: -20, dodge: -20 };
    case 'yellow':   return { aim: -10, dodge: -10 };
    case 'building': return { aim: +10, dodge: +15 };
    case 'wall':     return { aim: +15, dodge: +10 };
    case 'tower':    return { aim: +20, dodge: +15 };
...
export function computeHitChance(game, shooter, target){
  ...
  const aim   = 70 + s.aim + distancePenalty(shooter, target);
  const dodge = 10 + t.dodge;
...
export function formatUnitStatsHTML(u, game){
  ...
  const maxHp = Number.isFinite(u.maxHp) ? u.maxHp
               : (u.team === 'att' ? (game.attackersHP ?? 2000) : (game.defendersHP ?? 1700));
...
export function update(game){
  ...
  const phases = ['Twój Ruch (plan)','Twój Strzał','Twój Ruch (wyk.)','Strzał Wroga','Ruch Wroga'];
  const phText =
    game.mode === 'setup' ? 'Ustawienie' :
    game.over ? (game.winner === 'att' ? 'Przegrana' : 'Wygrana!') :
    phases[game.phase];
  const phi = document.getElementById('phaseInfo');
  if (phi) phi.textContent = phText;
  ...
  if (endBtn) {
      ...
      endBtn.disabled = !(game.mode === 'play' && game.phase === 0 && !game.over && !game.processingTurn);
  }
  ...
  const sumHp = (arr, game, side) => {
    ...
    for (const u of arr) {
      const baseMax = Number.isFinite(u.maxHp)
        ? u.maxHp
        : (side === 'att' ? (game.attackersHP ?? 2000) : (game.defendersHP ?? 1700));
  ...
  const { cur: attCur, max: attMax } = sumHp(game.attackers, game, 'att');
  const { cur: defCur, max: defMax } = sumHp(game.defenders, game, 'def');
  ...
  if (tbEnd){
    const enabled = (game.mode === 'play' && game.phase === 0 && !game.over && !game.processingTurn);
...
export function draw(game, opts = {}){
	...
	if (game.noAnimation) return;
	
  if (!textures.outside || !textures.inside || !textures.wood) {
    buildTextures();
  }
  ...
  for (const id in sectors) {
    ...
    ctx.save();
    if (s.type === 'tower' || s.type === 'wall' || s.type === 'building') {
      ctx.fillStyle = textures.wood;
      const vertical = (s.type !== 'building');
      const dmg = (s.type === 'building') ? 1 : 2;
      for (const [x,y] of cells) {
        ctx.fillRect(x*CELL, y*CELL, CELL, CELL);
    ...
    if (s.type === 'flag') drawAnimatedFlag(game);
  }
  ...
  for (const id in sectors){
    ...
    if (counts.attackers > 0 && game.mode !== 'setup'){
  ...
  // 4) Jednostki
  const units = (game.mode === 'setup') ? game.defenders : [...game.defenders, ...game.attackers];
  /*
  console.log("TRENING LOG:");
  console.log("game.plannedMoves");
	console.log(game.plannedMoves);
	console.log("game.movingUnits");
	console.log(game.movingUnits);
	*/
  for (const u of units){
    ...
    if (game.movingUnits[u.id]){
      const m = game.movingUnits[u.id], t = m.t;
      px = m.from.x + (m.to.x - m.from.x)*t;
      py = m.from.y + (m.to.y - m.from.y)*t;
    }

    const baseCol = (u.team === 'att') ? '#ff4444' : '#4444ff';
    let fillCol = baseCol;
  ...
  if (game.mode === 'play' && game.phase === 0 && !game.over && Object.keys(game.movingUnits).length === 0){
    const pm = game.plannedMoves || {};
    ...
    for (const uid of Object.keys(pm)){
      ...
      const gradCol = getPlanArrowGradient('def', meta);
  ...
  // 8) Highlight trafionej jednostki
  if (game.highlightedUnit && Date.now() < game.highlightUntil){
    const u = game.highlightedUnit;
    ctx.fillStyle = 'rgba(255,255,100,0.5)';
    ctx.fillRect(u.x*CELL, u.y*CELL, CELL, CELL);
    ctx.strokeStyle = '#ff0'; ctx.lineWidth = 4;
    ctx.strokeRect(u.x*CELL, u.y*CELL, CELL, CELL);
  } else if (game.highlightedUnit && Date.now() >= game.highlightUntil){
    game.highlightedUnit = null;
    game.highlightUntil = 0;
  }
...
export function drawAIPlans(game, ctx){
  if (game.mode !== 'play' || game.phase !== 0) return;
  if (game.showAIPlans === false) return;

  const plans = game.turnPlans?.att;
  ...
  for (const id of Object.keys(plans)){
    ...
    const gradCol = getPlanArrowGradient('att', p);
